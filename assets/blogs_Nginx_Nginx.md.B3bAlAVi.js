import{_ as n,o as a,c as p,at as l}from"./chunks/framework.CY22Hent.js";const e="/assets/Nginx_img_8.BaVeij2x.png",i="/assets/Nginx_img_9.BUwTFgFd.png",t="/assets/Nginx_img_10.CeHEfYYY.png",c="/assets/Nginx_img_11.r4CDLpCB.png",v=JSON.parse('{"title":"Nginx基础","description":"","frontmatter":{"title":"Nginx基础","date":"2018-08-12T00:00:00.000Z","lastUpdate":"2026-02-07T00:00:00.000Z","categories":["服务器"],"tags":["Nginx"]},"headers":[],"relativePath":"blogs/Nginx/Nginx.md","filePath":"blogs/Nginx/Nginx.md"}'),o={name:"blogs/Nginx/Nginx.md"};function r(h,s,d,g,x,u){return a(),p("div",null,[...s[0]||(s[0]=[l('<p>[toc]</p><h2 id="nginx基础" tabindex="-1">Nginx基础 <a class="header-anchor" href="#nginx基础" aria-label="Permalink to &quot;Nginx基础&quot;">​</a></h2><p><span style="color:red;">基于当前nginx 1.16.0 版本所述。修改日期-20191223</span></p><p>Nginx是一款轻量级的Web 服务器,==反向代理服务器==,电子邮件（IMAP/POP3）代理服务器</p><blockquote><p>Nginx的应用场景</p></blockquote><ol><li>http服务器。Nginx是一个http服务可以独立提供http服务。可以做网页静态服务器。</li><li>虚拟主机。可以实现在一台服务器虚拟出多个网站。例如个人网站使用的虚拟主机。</li><li>反向代理，负载均衡。</li></ol><h4>Nginx的负载均衡</h4><blockquote><p>负载：就是Nginx接受请求. 均衡：Nginx将收到的请求按照一定的规则分发到不同的服务器进行处理。</p></blockquote><p><img src="'+e+'" alt="8" loading="lazy"></p><p>nginx支持的负载均衡调度算法方式如下：</p><ol><li><p>weight轮询（默认）：把接收到的请求按照顺序逐一分配到不同的服务器上，即使某一台服务器宕机，nginx会自动将该服务器剔除出队列，请求受理情况不会受到任何影响。 或者可以给不同的后端服务器设置一个权重值（weight），权重数据越大，被分配到请求的几率越大。</p></li><li><p>ip_hash：每个请求按照发起客户端的ip的hash结果进行匹配，这样的算法下一个固定ip地址的客户端总会访问到同一个后端服务器。</p></li><li><p>fair：智能调整调度算法，动态的根据后端服务器的请求处理到响应的时间进行均衡分配，响应时间短处理效率高的服务器分配到请求的概率高，响应时间长处理效率低的服务器分配到的请求少；需要注意的是nginx默认不支持fair算法，如果要使用这种调度算法，请安装upstream_fair模块</p></li><li><p>url_hash：按照访问的url的hash结果分配请求，每个请求的url会指向后端固定的某个服务器，可以在nginx作为静态服务器的情况下提高缓存效率。同样注意nginx默认不支持这种调度算法，要使用的话需要安装nginx的hash软件包</p></li></ol><h3 id="_1-什么是反向代理" tabindex="-1">1.什么是反向代理？ <a class="header-anchor" href="#_1-什么是反向代理" aria-label="Permalink to &quot;1.什么是反向代理？&quot;">​</a></h3><h4>正向代理</h4><p>当客户端想要间接访问一个目标服务器时。客户端可以找一个可以访问目标服务器的另外一台服务器。这个另外一台服务器就是代理服务器。</p><p>正向代理的过程：</p><ol><li>客户端把请求发送给代理服务器，再通过代理服务器把请求发送给目标服务器。</li><li>代理服务器从目标服务器处获得相应的数据后，再转发给客户端。</li></ol><p><span style="color:red;">在正向代理的过程中，客户端和代理服务器知道目标服务器的IP地址，而目标服务器只知道代理服务器的IP地址而不知道客户端的IP地址。所以正向代理可以屏蔽或隐藏客户端的信息。所以代理服务器是为客户端作代理人，它是站在客户端这边的</span></p><p><img src="'+i+'" alt="9" loading="lazy"></p><h4>反向代理</h4><p><span style="color:red;">由于正向代理是代理服务器作为客户端的代理人（客户端与代理是一伙的）。而反向代理是指代理服务器作为服务器的代理人（代理与目标服务器是一伙的）。并且在反向代理过程中，屏蔽或隐藏目标服务器的信息</span></p><p><img src="'+t+'" alt="10" loading="lazy"></p><h4>一般在项目场景中</h4><p>在实际项目中，正向代理和反向代理很有可能会存在在一个应用场景中，正向代理代理客户端的请求去访问目标服务器，目标服务器是一个反向单利服务器，反向代理了多台真实的业务处理服务器。</p><p><img src="'+c+`" alt="11" loading="lazy"></p><h3 id="_2-对nginx进行配置" tabindex="-1">2.对Nginx进行配置 <a class="header-anchor" href="#_2-对nginx进行配置" aria-label="Permalink to &quot;2.对Nginx进行配置&quot;">​</a></h3><p>nginx使用最多的三个核心功能是反向代理、负载均衡和静态服务器。这三个不同的功能的使用，都要从nginx的配置文件nginx.conf中进行配置。</p><p>上面的配置文件由5个部分组成：</p><p>main模块：用于nginx全局信息的配置,不在event.http括号中的配置信息 events模块：用于nginx工作模式的配置 http模块：用于http协议信息的一些配置 server模块：用于服务器访问信息的配置 location模块：用于进行访问路由的配置</p><blockquote><p>重点server模块是http模块中的一个子模块，用于服务器访问信息的配置</p></blockquote><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>server {                      #一个虚拟主机的配置，一个http中可以配置多个server</span></span>
